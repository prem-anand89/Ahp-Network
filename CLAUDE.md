...
`ARCHITECTURE_REVIEW.md` records the pre-code review that produced the v19 corrections: what was found wrong or unbuildable in v18, why each fix was chosen, and — in its section E — the **open decisions that block specific phases** (UI stack, the encryption envelope's call site, Metabase hosting, the verified-only filter default, whether clinic referrals are in pilot scope). Check section E before starting a phase; if the phase you're on has an unresolved decision listed against it, flag it rather than picking one yourself.

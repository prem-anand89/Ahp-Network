...
| Connection pooling | **Cloudflare Hyperdrive for every query, including the referral transactions.** Hyperdrive is transaction-mode only; the referral transactions are safe over it because each is a *single* `SELECT fn(...)` statement (see the referral-transaction rule below), and a single statement is atomic by definition. No session-mode pooler, no second connection path. |

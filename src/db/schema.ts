...
export const homeCaseReferrals = pgTable(
  "home_case_referrals",
  {
    ...
    // a community the poster belongs to, or one named therapist (the
    // "Refer Patient" profile CTA). circle_first_window/circleFirstOpenedAt
    // keep their names and now gate all three; a referral is "in First
    // Look" exactly when circleFirstWindow is set and circleFirstOpenedAt
    // isn't. At most one of the three target columns is ever set — see
    // the CHECK constraints below, which make that structural rather than
    // application discipline.
    firstLookCommunityId: uuid("first_look_community_id").references(() => communities.id),
    firstLookTherapistId: uuid("first_look_therapist_id").references(() => users.id),
    // Whether the scheduler opens First Look to the full matched pool once
    // the window passes — circle-first always did (true); infrastructure
    // for a future "don't expand" choice, not yet exposed in any UI.
    expandToNetwork: boolean("expand_to_network").notNull().default(true),
    // Phase 5 — the referral receipt. Set once, the moment a referral
    // actually reaches 'completed' (referral-outcomes.ts) — format
    // R-YYYY-NNNN, generated by referral-receipt.ts. Per-referral, never
    // per-person: there is no column or query anywhere that counts these
    // per user — "you've completed 7" on any public surface is exactly
    // the comparison §1A bans; a receipt is a record of one specific
    // professional handoff, not a running total.
    publicRefCode: text("public_ref_code"),
    deletedAt: timestamp("deleted_at", { withTimezone: true }),
    createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
    updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
  },
  (table) => [
    ...
    uniqueIndex("home_case_referrals_public_ref_code")
      .on(table.publicRefCode)
      .where(sql`${table.publicRefCode} IS NOT NULL`),
    check(

...
// Regression guard for the actual bug, not just a comment: the cache TTL
// must stay below postgres.js's idle_timeout (20s in db.ts), or getDb()
// can hand back a client whose socket is already closed, surfacing as
// React error #412 ("Connection closed") on ordinary /app/* navigation.
describe("db client reuse window", () => {
  ...
  it("keeps the reuse window below postgres.js's idle_timeout (20s)", () => {

...
| 5 | VAPID signing on Workers | **Built in Phase 7** (`src/lib/web-push.ts`), via `@block65/webcrypto-web-push` — a small library implementing RFC 8291/8292 over the standard WebCrypto API specifically for Workers/Deno/Bun, rather than the `web-push` npm package (Node's `crypto` module, confirmed not Workers-compatible). VAPID keys generated locally. **Still unverified against a live push endpoint** — no real device/browser subscription was available to test the actual FCM/Apple round trip from a deployed Worker; only the send/parallel-email decision logic is unit-tested. Same honesty standard as item 4's note in `src/lib/ocr/vision.ts`. |
